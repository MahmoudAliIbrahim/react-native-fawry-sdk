...
public class ConvertUtils {
    ...
    public static List<PayableItem> objectListToPayableItems(List<Map> items){
        List<PayableItem> payableItems = new ArrayList<>();
        for (int i = 0; i < items.size(); i++) {
            Item item = new Item();
            Map oItem = items.get(i);
            item.setPrice(oItem.get("price").toString());
            item.setDescription(oItem.get("description").toString());
            item.setQty(oItem.get("quantity").toString());
            item.setSku(oItem.get("sku").toString());
            Log.d("FAWRY", "objectListToPayableItems: " + item.getFawryItemDescription());
